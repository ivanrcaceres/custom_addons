odoo.define('pos_kitchen.kitchen_kanban', function (require) {
    ...
    var KitchenKanban = AbstractAction.extend({
        ...
        events: {
            ...
            'click .icon-close': '_onHideRecipie',
        ...
        load_data: function () {
            ...
            today = yyyy + '-' + mm + '-' + dd;
            var fields = ['id', 'kitchen_state', 'name', 'start_date'];
            self._rpc({
                model: 'pos.order.line',
                method: 'search_read',
                args: [[['create_date', '>=', today], ['product_id.pos_categ_id.id', 'in', self.category_ids]], ['kitchen_state', 'qty', 'order_id', 'product_id', 'avg_completion_time']],
            }).then(function(lines) {
                lines.forEach((line) => {
                    line.full_product_name = line.product_id[1]
                });
                self._rpc({
                    model: 'pos.order',
                    method: 'search_read',
        ...
        _onShowReceipie: function (ev) {
            ...
            this._rpc({
                ...
                method: 'search_read',
                args: [[['id', '=', product]], ['name', 'recipie', 'image']],
            }).then(function (data) {
                if (!data[0]['recipie']) {
                    data[0]['recipie'] = '';
                }
                ...
                if (data[0]['image'] !== false) {
                    html_data += '<p class="product_image"><img src="data:image/png;base64, ' + data[0]['image'] + '" /></p>';
                }
